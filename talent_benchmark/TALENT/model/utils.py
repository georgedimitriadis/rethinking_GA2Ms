...
def get_classical_args():
    ...
    parser.add_argument('--model_type', type=str,
                        ...
                        choices=['LogReg', 'NCM', 'RandomForest',
                                 ...
                                 'svm', 'knn', 'NaiveBayes', "dummy", "LinearRegression", "ecmac", "ecmac_sr"
                                 ])
...
def show_results_classical(args, info, metric_name, results_list, time_list):
    ...
    for name in metric_name:
        if info['task_type'] == 'regression' and (name != 'Time' or name != 'OOD_DATA_BOUNDS' or name != 'OOD_DATA_RMSE' or name != 'OOD_DATA_R2'):
...
def show_results_ood(args, info, metric_name, results_list, time_list, ood_measure_names, ood_vres_list):
    show_results_classical(args, info, metric_name, results_list, time_list)

    for i in range(len(ood_measure_names)):
        print(f'{ood_measure_names[i]}: {", ".join([f"{e[i]}" for e in ood_vres_list])}')
...
def tune_hyper_parameters(args, opt_space, train_val_data, info):
    ...
    def objective(trial):
        ...
        if args.model_type in ['ecmac', 'ecmac_sr']:
            config['model']['ec']['epochs'] = 5000
            config['model']['ec']['learning_rate'] = 0.001
            config['model']['ec']['batch_size'] = 16
            config['model']['ec']['validation_split'] = 0.1
            config['model']['ec']['arity'] = 2
            config['model']['ec']['final_linear_layer_regularizer'] = "None"
...
def get_method(model):
    ...
    if model == "mlp":
        ...
        from TALENT.model.classical_methods.ecmac import ECMACMethod
        return ECMACMethod
    elif model == 'ecmac_sr':
        from TALENT.model.classical_methods.ecmac_sr import ECMACMethod
        return ECMACMethod
    else:
